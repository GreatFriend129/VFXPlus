...
namespace VFXPlus.Content.VFXTest.Aero.CavernSword
{
    public class CavernSword : ModItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            if (player.altFunctionUse != 2)
                type = ModContent.ProjectileType<CavernSwordProj>(); //Alt is slower
            else
    ...
    public class CavernSwordProj : BaseSwingSwordProj
    {
        ...
        {
            SwingHalfAngle = 155; //125 | 185 not bad
            easingAdditionAmount = 0.075f / Projectile.extraUpdates; //0.09
            frameToStartSwing = 0 * Projectile.extraUpdates; //0
            timeAfterEnd = 1 * Projectile.extraUpdates; //1
        ...
        {
            Texture2D Texture = Mod.Assets.Request<Texture2D>("Content/VFXTest/Aero/CavernSword/CavernSword").Value;
            ...
            if (getProgress(easingProgress) > 0.0f && getProgress(easingProgress) < 0.99f)
            {
                ...
                float swingTotalScale = 1f;

                Main.spriteBatch.Draw(SwingTex, swingTexPos, null, rainbowCol * ((float)Math.Sin(getProgress(easingProgress) * Math.PI) * 0.75f), swingRot, SwingTex.Size() / 2, slashScales[0] * swingTotalScale, SpriteEffects.None, 0f);
                Main.spriteBatch.Draw(SwingTex, swingTexPos, null, rainbowCol * ((float)Math.Sin(getProgress(easingProgress) * Math.PI) * 0.5f), swingRot, SwingTex.Size() / 2, slashScales[1] * swingTotalScale, SpriteEffects.None, 0f);
                Main.spriteBatch.Draw(SwingTex, swingTexPos, null, rainbowCol * ((float)Math.Sin(getProgress(easingProgress) * Math.PI) * 0.3f), swingRot, SwingTex.Size() / 2, slashScales[2] * swingTotalScale, SpriteEffects.None, 0f);
            }
            #endregion
        ...
        public void TrailDraw(bool giveUp)
        {
            ...
            trailEffect.Parameters["glowThreshold"].SetValue(1f);
            trailEffect.Parameters["glowIntensity"].SetValue(1f);
            trailEffect.Parameters["ColorOne"].SetValue(Color.SkyBlue.ToVector3() * 3.5f);
            trailEffect.CurrentTechnique.Passes["MainPS"].Apply();
    ...
    public class CavernSwordElecChain : ModProjectile
    {
        ...
        public override void AI()
        {
            if (timer == 0)
            {
                ...
                SoundStyle stylea = new SoundStyle("VFXPlus/Sounds/Effects/lightning_flash_01") with { Volume = 0.08f, Pitch = 1f, PitchVariance = 0.2f, MaxInstances = -1 };
