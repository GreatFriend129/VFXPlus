...
namespace VFXPlus.Content.Weapons.Magic.Hardmode.Misc
{
    public class LifeDrain : GlobalItem 
    {
        ...
        public override bool Shoot(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            for (int i = 220; i < 10; i++) //16
            {
                ...
                Dust d = Dust.NewDustPerfect(Main.MouseWorld + new Vector2(0f, -100), ModContent.DustType<MediumSmoke>(), Velocity: Main.rand.NextVector2Unit() * Main.rand.NextFloat(0.5f, 3.4f) * 1.65f,
                    newColor: col with { A = 50 }, Scale: Main.rand.NextFloat(0.9f, 1.5f) * 1.5f);
    ...
    public class LifeDrainVFX : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.EntitySpriteDraw(orb, drawPos, null, cols[0] with { A = 30 } * orbAlpha, 0f, orbOrigin, orbScale * scales[0], SpriteEffects.None);
            Main.EntitySpriteDraw(orb, drawPos, null, cols[1] with { A = 30 } * orbAlpha, 0f, orbOrigin, orbScale * scales[1] * sineScale1, SpriteEffects.None);
            Main.EntitySpriteDraw(orb, drawPos, null, cols[2] with { A = 30 } * orbAlpha, 0f, orbOrigin, orbScale * scales[2] * sineScale2, SpriteEffects.None);
        ...
        public void GoddamnMonsoon(int count = 50)
        {
            ...
            float speedTime = Main.GlobalTimeWrappedHourly * 1.1f * Projectile.spriteDirection;
            Vector2 scale = new Vector2(0.26f, 0.55f) * 0.4f;


            float minRange = 35f * drawScale;
            float maxRange = 105f * drawScale;
            for (int i = 0; i < count; i++)
            {
                ...
                Color col = new Color(255, 50, 50);

                Main.EntitySpriteDraw(Tex, drawPosition, null, col with { A = 60 } * drawAlpha, randomRot + MathHelper.PiOver2, Tex.Size() / 2f,
