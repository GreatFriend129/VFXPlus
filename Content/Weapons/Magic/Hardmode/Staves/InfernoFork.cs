...
namespace VFXPlus.Content.Weapons.Magic.Hardmode.Staves
{
    ...
    public class InfernoBlastOverride : GlobalProjectile
    {
        ...
        public override bool PreAI(Projectile projectile)
        {
            if (timer == 0)
            {
                //Store the index of the VFXproj in ai[2]
                if (Main.myPlayer == projectile.owner)
                    projectile.ai[2] = Projectile.NewProjectile(projectile.GetSource_FromThis(), projectile.Center, Vector2.Zero, ModContent.ProjectileType<InfernoForkVFX>(), 0, 0, projectile.owner);
        ...
        public override bool PreKill(Projectile projectile, int timeLeft)
        {
            ...
            if (vfxProj.active && vfxProj.type == ModContent.ProjectileType<InfernoForkVFX>())
            {
                (vfxProj.ModProjectile as InfernoForkVFX).shouldFadeOut = true;
            }
