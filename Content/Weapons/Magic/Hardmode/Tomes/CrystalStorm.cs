...
namespace VFXPlus.Content.Weapons.Magic.Hardmode.Tomes
{
    ...
    public class CrystalStormShotOverride : GlobalProjectile
    {
        ...
        public override bool PreAI(Projectile projectile)
        {
            ...
            //Smooth out the trail a bit if we just tileCollided
            if (justTileCollided && timer > 0 && trail1.trailRotations.Count >= 1)
            {
                ...
                trail1.trailPositions.Add(projectile.Center);
            ...
            //Dust
            if (timer % 2 == 0 && shouldSpawnDust && timer > 5)
            {
            ...
            float timeForSpinIn = Math.Clamp((float)timer / 40f, 0f, 1f); //40
            fadeInPower = Easings.easeOutSine(timeForSpinIn);

            overallAlpha = Math.Clamp(MathHelper.Lerp(overallAlpha, 1.5f, 0.11f), 0f, 1f);

            float timeForPopInAnim = 35;
            float animProgress = Math.Clamp((timer + 15) / timeForPopInAnim, 0f, 1f);
            overallScale = MathHelper.Lerp(0f, 1f, Easings.easeInOutBack(animProgress, 0f, 2f));
        ...
        public override bool PreDraw(Projectile projectile, ref Color lightColor)
        {
            ...
            //Star
            float starAlpha = (1f - fadeInPower) * 1f * overallAlpha;
            float spinInBonusRot = MathHelper.Lerp(7f * projectile.direction * -1, 0f, Easings.easeOutQuad(fadeInPower));

            Vector2 vec2Scale = new Vector2(0.6f, 0.6f) * projectile.scale * (1f - (fadeInPower * 0.75f)) * overallScale;

            Main.EntitySpriteDraw(Star, DrawPos, null, Color.Black * starAlpha, spinInBonusRot, Star.Size() / 2f, vec2Scale * 0.75f, SpriteEffects.None);
            Main.EntitySpriteDraw(Star, DrawPos, null, colToUse with { A = 0 } * starAlpha, spinInBonusRot, Star.Size() / 2f, vec2Scale * 1f, SpriteEffects.None);
            Main.EntitySpriteDraw(Star, DrawPos, null, Color.White with { A = 0 } * starAlpha, spinInBonusRot, Star.Size() / 2f, vec2Scale * 0.85f, SpriteEffects.None);
            ...
            for (int i = 0; i < 6; i++) //8
            {
                ...
                Main.EntitySpriteDraw(TexToUse, DrawPos + offset, null, Color.White with { A = 0 } * 2f * overallAlpha, projectile.rotation, TexOrigin, projectile.scale * overallScale, SpriteEffects.None);
            }
        ...
        public override bool OnTileCollide(Projectile projectile, Vector2 oldVelocity)
        {
            ...
            for (int i = 0; i < 3 + Main.rand.Next(1, 3); i++)
            {
                ...
                Dust p = Dust.NewDustPerfect(projectile.Center + oldVelocity * 0.5f, ModContent.DustType<GlowPixelCross>(), vel * Main.rand.NextFloat(0.8f, 1.05f),
                    newColor: col * 0.5f, Scale: Main.rand.NextFloat(0.15f, 0.3f) * projectile.scale);
            }
