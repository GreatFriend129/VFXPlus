    ...
    public class VilethornBaseShotOverride : GlobalProjectile
    {
        ...
        public override bool PreAI(Projectile projectile)
        {
            if (timer == 8)
            {
                ...
                SoundStyle style = new SoundStyle("VFXPlus/Sounds/Effects/Metallic/joker_stab2") with { Volume = .01f, Pitch = pitch2, PitchVariance = .05f, MaxInstances = -1, };
                ...
                SoundStyle style3 = new SoundStyle("VFXPlus/Sounds/Effects/Earth/PlantGrowth") with { Volume = 0.1f, Pitch = pitch3, PitchVariance = 0.05f, MaxInstances = -1 };
            ...
            if (timer >= 2 && timer <= 9 && timer % 3 == 0) 
            {
                ...
                Dust p = Dust.NewDustPerfect(projectile.Center + posOffset, ModContent.DustType<PulseInOutDust>(), vel * Main.rand.NextFloat(0.8f, 1.05f),
                    newColor: new Color(163, 51, 255) with { A = 20 }, Scale: Main.rand.NextFloat(0.4f, 0.5f) * projectile.scale * 1.25f);
                p.velocity += (projectile.rotation + MathHelper.PiOver2).ToRotationVector2() * -1f;
                p.velocity *= 0.85f;
                p.rotation = MathHelper.PiOver4;
                p.noLight = false;

                p.customData = new PulseInOutDustBehavior(PulseInOutDustBehavior.DrawOptions.GlowPixel, 18, 0.5f, 0.5f, true);
            }
