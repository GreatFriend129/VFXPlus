...
namespace VFXPlus.Content.Weapons.Ranged.Ammo.Bullets
{
    ...
    public class GoldenBulletProjOverride : GlobalProjectile, IDrawAdditive
    {
        ...
        public override bool PreAI(Projectile projectile)
        {
            ...
            trail1.trailMaxLength = 150 + trailRandomLengthOffset; //120

            trail1.shouldSmooth = false;

            Color heartOfGold = new Color(232, 186, 88);
            trail1.trailColor = new Color(232, 186, 88) * totalAlpha; //Color.DarkGoldenrod
            ...
            if (timer > 5 && timer % 4 == 0 && Main.rand.NextBool(3))
            {
                ...
                Dust d = Dust.NewDustPerfect(projectile.Center, ModContent.DustType<GlowPixelAlts>(), vel, newColor: Color.Gold * 1f, Scale: Main.rand.NextFloat(0.45f, 0.5f) * 0.4f);
                d.alpha = 2;
                d.velocity += -projectile.velocity.RotatedByRandom(0.1f) * 0.55f;
                d.velocity *= 0.35f;
            }

            if (timer > 5 && timer % 7 == 0 && Main.rand.NextBool(3))
            {
                Vector2 vel = Main.rand.NextVector2Circular(3f, 3f);

                Dust d = Dust.NewDustPerfect(projectile.Center, ModContent.DustType<GlowPixelCross>(), vel, newColor: heartOfGold, Scale: Main.rand.NextFloat(0.45f, 0.5f) * 0.65f);
                d.velocity += -projectile.velocity.RotatedByRandom(0.1f) * 0.55f;
                d.velocity *= 0.35f;
                d.customData = DustBehaviorUtil.AssignBehavior_GPCBase(rotPower: 0.15f, fadePower: 0.9f, shouldFadeColor: false);
            }
            ...
            totalScale = 0f + MathHelper.Lerp(0f, 1f, Easings.easeInOutBack(animProgress, 0f, 2f)) * 1f;

            Lighting.AddLight(projectile.Center, heartOfGold.ToVector3() * 0.55f);
        ...
        public override bool PreDraw(Projectile projectile, ref Color lightColor)
        {
            ...
            Color spikeColor = new Color(227, 170, 49);
            ...
            Color orbColor = Color.Lerp(Color.DarkGoldenrod, Color.Orange, 1f);
            ...
            Texture2D star = ModContent.Request<Texture2D>("VFXPlus/Assets/Pixel/CrispStarPMA").Value;
            float starRot = (float)Main.timeForVisualEffects * (0.1f * randomTrailSpeed) * projectile.direction;

            Main.EntitySpriteDraw(star, drawPos, null, orbColor with { A = 0 } * 0.12f * totalAlpha, starRot, star.Size() / 2f, 1f, SpriteEffects.None);
        ...
        public void DrawAdditive(SpriteBatch sb)
        {
            ...
            Color outerSpikeCol = new Color(232, 186, 88);
            //Color outerSpikeCol2 = new Color(227, 170, 49);

            sb.Draw(spike, drawPos, null, outerSpikeCol * 1f * totalAlpha, drawRot, drawOrigin, drawScale, SpriteEffects.None, 0f);
        ...
        {            
            ...
            {
                Color crossCol = Color.Lerp(Color.Gold, Color.LightGoldenrodYellow, 0.4f);
                ...
                Dust p = Dust.NewDustPerfect(projectile.Center, ModContent.DustType<GlowPixelCross>(), dustVel, newColor: crossCol, Scale: Main.rand.NextFloat(0.2f, 0.4f) * 1.5f);
            ...
            for (int i = (int)(count * 0.3f); i < count; i += 5)
            {
                if (Main.rand.NextBool())
                {
                    ...
                    Dust d = Dust.NewDustPerfect(pos, ModContent.DustType<GlowPixelAlts>(), vel, newColor: Color.Gold, Scale: Main.rand.NextFloat(0.45f, 0.5f) * 0.4f);
                    d.alpha = 2;
