...
{
    public class SDMG : GlobalItem 
    {
        public override bool InstancePerEntity => true;
        public override bool AppliesToEntity(Item item, bool lateInstatiation)
        {
            return lateInstatiation && (item.type == ItemID.SDMG);
        }
        ...
        public override bool Shoot(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            Vector2 muzzlePos = position + new Vector2(56f, -2f * dir).RotatedBy(velocity.ToRotation()) + new Vector2(0f, 1f);
            ...
            //Light Dust
            //Dust softGlow = Dust.NewDustPerfect(muzzlePos, ModContent.DustType<SoftGlowDust>(), Vector2.Zero, newColor: Color.Aqua, Scale: 0.1f);
            ...
            for (int i = 0; i < 1 + Main.rand.Next(0, 3); i++)
            {
                ...
                dust.velocity += velocity.SafeNormalize(Vector2.UnitX) * 4f;
            }

            //Bullet Casing
            Gore.NewGore(source, position + velocity, new Vector2(velocity.X * -0.25f, -0.75f), ModContent.GoreType<AquaCasing>());
