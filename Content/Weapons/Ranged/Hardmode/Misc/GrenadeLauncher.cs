...
namespace VFXPlus.Content.Weapons.Ranged.Hardmode.Misc
{
    public class GrenadeLauncher : GlobalItem
    {
        ...
        {
            return lateInstatiation && (item.type == ItemID.GrenadeLauncher);
        }
        ...
        public override bool Shoot(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            {
                held.SetProjInfo(
                    GunID: ItemID.GrenadeLauncher,
                    AnimTime: 20,
                    NormalXOffset: 19f,
                    DestXOffset: -3f,
                    YRecoilAmount: 0.15f,
                    HoldOffset: new Vector2(0f, 0f),
                    TipPos: new Vector2(38f, -1f),
            ...
            Vector2 muzzlePos = position + new Vector2(40f, -2f * dir).RotatedBy(velocity.ToRotation()) + new Vector2(0f, 1f); //4448
            ...
            SoundStyle style4 = new SoundStyle("Terraria/Sounds/Item_61") with { Volume = 0.6f, PitchVariance = .12f, }; 
            SoundEngine.PlaySound(style4, player.Center);

            return true;
        }
    }
    public class GrenadeLauncherShotOverride : GlobalProjectile
    {
        public override bool InstancePerEntity => true;
        public override bool AppliesToEntity(Projectile entity, bool lateInstantiation)
        {
            bool rocket12 = entity.type == ProjectileID.GrenadeI || entity.type == ProjectileID.GrenadeII;
            bool rocket34 = entity.type == ProjectileID.GrenadeIII || entity.type == ProjectileID.GrenadeIV;
            bool rocketNuke = entity.type == ProjectileID.MiniNukeGrenadeI || entity.type == ProjectileID.MiniNukeGrenadeII;
            bool rocketCluster = entity.type == ProjectileID.ClusterGrenadeI || entity.type == ProjectileID.ClusterGrenadeII;
        ...
        public override bool PreDraw(Projectile projectile, ref Color lightColor)
        {
            ...
            float Xscale = Math.Clamp(projectile.velocity.Length() / 10f, 0f, 1f);
            ...
            for (int i = 0; i < previousRotations.Count; i++)
            {
                ...
                Vector2 flareScale = new Vector2(1f * Xscale, 0.55f * progress) * scale;

                //Make the frontmost flare have less X length
            ...
            Main.EntitySpriteDraw(Ball, ballOff1, null, Color.OrangeRed with { A = 0 } * 0.35f, projectile.velocity.ToRotation() + MathHelper.PiOver2, Ball.Size() / 2f, new Vector2(0.7f * Xscale, 0.6f) * projectile.scale * overallScale, SE);
        ...
        public override bool PreKill(Projectile projectile, int timeLeft)
        {
            ...
            else if (projectile.type == ProjectileID.GrenadeIII || projectile.type == ProjectileID.GrenadeIV)
            {
                ProjectileExtensions.DoBaseRocketIIIExplosionFX(projectile, storedPosition);

                if (projectile.type == ProjectileID.GrenadeIV && projectile.owner == Main.myPlayer)
                    TileExplosion(projectile);
            }
            else if (projectile.type == ProjectileID.MiniNukeGrenadeI || projectile.type == ProjectileID.MiniNukeGrenadeII)
            {
                ProjectileExtensions.DoBaseMiniNukeExplosionFX(projectile, storedPosition);

                if (projectile.type == ProjectileID.MiniNukeGrenadeII && projectile.owner == Main.myPlayer)
                    TileExplosion(projectile);
            }
            else if (projectile.type == ProjectileID.ClusterGrenadeI || projectile.type == ProjectileID.ClusterGrenadeII)
            {
                ProjectileExtensions.DoBaseClusterRocketExplosionFX(projectile, storedPosition);

                if (projectile.type == ProjectileID.ClusterGrenadeII && projectile.owner == Main.myPlayer)
                    TileExplosion(projectile);
    ...
    public class LiquidGrenadesShotOverride : GlobalProjectile
    {
        public override bool InstancePerEntity => true;
        public override bool AppliesToEntity(Projectile entity, bool lateInstantiation)
        {
            bool check = entity.type == ProjectileID.DryGrenade || entity.type == ProjectileID.WetGrenade
                || entity.type == ProjectileID.LavaGrenade || entity.type == ProjectileID.HoneyGrenade;
        ...
        public override bool PreAI(Projectile projectile)
        {
            ...
            if (projectile.type == ProjectileID.WetGrenade)
            {
                col1 = Color.DeepSkyBlue;
                col2 = Color.DodgerBlue;
            }
            if (projectile.type == ProjectileID.LavaGrenade)
            {
                col1 = Color.Orange;
                col2 = Color.OrangeRed;
            }
            if (projectile.type == ProjectileID.HoneyGrenade)
            {
                col1 = Color.Gold;
                col2 = Color.Orange;
            }
        ...
        public override bool PreDraw(Projectile projectile, ref Color lightColor)
        {
            ...
            Main.EntitySpriteDraw(Ball, ballOff1, null, col2 with { A = 0 } * 0.35f, projectile.velocity.ToRotation() + MathHelper.PiOver2, Ball.Size() / 2f, new Vector2(0.7f * Xscale, 0.6f) * projectile.scale * overallScale, SE);
        ...
        public override bool PreKill(Projectile projectile, int timeLeft)
        {
            if (projectile.type == ProjectileID.DryGrenade)
            {
            ...
            else if (projectile.type == ProjectileID.WetGrenade)
            {
            ...
            else if (projectile.type == ProjectileID.LavaGrenade)
            {
            ...
            else if (projectile.type == ProjectileID.HoneyGrenade)
            {
            ...
            SoundStyle style = new SoundStyle("VFXPlus/Sounds/Effects/water_blast_projectile_spell_03") with { Volume = .1f, Pitch = -.05f, PitchVariance = 0.05f, MaxInstances = -1 };
