...
namespace VFXPlus.Content.Weapons.Ranged.Hardmode.Misc
{
    public class RocketLauncherOverride : GlobalItem
    {
        ...
        {
            return lateInstatiation && (item.type == ItemID.RocketLauncher);
        }
        ...
        public override bool Shoot(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            Dust d2 = Dust.NewDustPerfect(position + velocity * 2f, ModContent.DustType<CirclePulse>(), velocity * 0.75f, newColor: Color.OrangeRed);
            ...
            //SoundStyle style2 = new SoundStyle("AerovelenceMod/Sounds/Effects/TF2/rescue_ranger_fire") with { Volume = .04f, Pitch = .65f, PitchVariance = .05f, MaxInstances = 1 };
            //SoundEngine.PlaySound(style2, player.Center);

            return true;
        }
    }
    public class RocketLauncherShotOverride : GlobalProjectile
    {
        public override bool InstancePerEntity => true;
        public override bool AppliesToEntity(Projectile entity, bool lateInstantiation)
        {
            bool rocket12 = entity.type == ProjectileID.RocketI || entity.type == ProjectileID.RocketII;
            bool rocket34 = entity.type == ProjectileID.RocketIII || entity.type == ProjectileID.RocketIV;
            bool rocketNuke = entity.type == ProjectileID.MiniNukeRocketI || entity.type == ProjectileID.MiniNukeRocketII;
            bool rocketCluster = entity.type == ProjectileID.ClusterRocketI || entity.type == ProjectileID.ClusterRocketII;

            return lateInstantiation && (rocket12 || rocket34 || rocketNuke || rocketCluster);
        }
        ...
        public override bool PreAI(Projectile projectile)
        {
            ...
            if (timer % 3 == 0 && timer > 10)
            {
                ...
                Color dustCol = Color.Lerp(Color.OrangeRed, Color.Orange, 0.3f);
        ...
        public override bool PreDraw(Projectile projectile, ref Color lightColor)
        {
            ...
            Main.EntitySpriteDraw(Ball, ballOff1, null, Color.OrangeRed with { A = 0 } * 0.35f, projectile.rotation, Ball.Size() / 2f, new Vector2(0.7f, 0.7f) * projectile.scale * overallScale, SE);
        ...
        public void DrawTrail(Projectile projectile, bool giveUp)
        {
            ...
            Color col = Color.Lerp(Color.Orange, Color.OrangeRed, 0.05f);
            ...
            for (int i = 0; i < previousRotations.Count; i++)
            {
                ...
                //Start End
                Color trailCol = Color.Lerp(col, Color.Red, 1f - progress) * progress;
        ...
        public override bool PreKill(Projectile projectile, int timeLeft)
        {
            ...
            else if (projectile.type == ProjectileID.RocketIII || projectile.type == ProjectileID.RocketIV)
            {
                ProjectileExtensions.DoBaseRocketIIIExplosionFX(projectile, storedPosition);

                if (projectile.type == ProjectileID.RocketIV && projectile.owner == Main.myPlayer)
                    TileExplosion(projectile);
            }
            else if (projectile.type == ProjectileID.MiniNukeRocketI || projectile.type == ProjectileID.MiniNukeRocketII)
            {
                ProjectileExtensions.DoBaseMiniNukeExplosionFX(projectile, storedPosition);

                if (projectile.type == ProjectileID.MiniNukeRocketII && projectile.owner == Main.myPlayer)
                    TileExplosion(projectile);
            }
            else if (projectile.type == ProjectileID.ClusterRocketI || projectile.type == ProjectileID.ClusterRocketII)
            {
                ProjectileExtensions.DoBaseClusterRocketExplosionFX(projectile, storedPosition);

                if (projectile.type == ProjectileID.ClusterRocketII && projectile.owner == Main.myPlayer)
                    TileExplosion(projectile);
    ...
    public class LiquidRocketsShotOverride : GlobalProjectile
    {
        public override bool InstancePerEntity => true;
        public override bool AppliesToEntity(Projectile entity, bool lateInstantiation)
        {
            bool check = entity.type == ProjectileID.DryRocket || entity.type == ProjectileID.WetRocket
                || entity.type == ProjectileID.LavaRocket || entity.type == ProjectileID.HoneyRocket;

            return lateInstantiation && check;
        }
        ...
        public override bool PreAI(Projectile projectile)
        {
            ...
            Color col1 = Color.Silver;
            Color col2 = Color.Silver;

            if (projectile.type == ProjectileID.WetRocket)
            {
                col1 = Color.DeepSkyBlue;
                col2 = Color.DodgerBlue;
            }
            if (projectile.type == ProjectileID.LavaRocket)
            {
                col1 = Color.Orange;
                col2 = Color.OrangeRed;
            }
            if (projectile.type == ProjectileID.HoneyRocket)
            {
                col1 = Color.Gold;
                col2 = Color.Orange;
            }
            ...
            if (timer % 3 == 0 && timer > 10)
            {
                ...
                Dust smoke = Dust.NewDustPerfect(dustPos, ModContent.DustType<GlowPixelAlts>(), dustVel, newColor: col1 * 1f, Scale: dustScale);
                smoke.alpha = 2;
            }
        ...
        public override bool PreDraw(Projectile projectile, ref Color lightColor)
        {
            ...
            for (int i = 0; i < previousRotations.Count; i++)
            {
                ...
                //Start End
                Color trailCol = Color.Lerp(col1, col2, 1f - progress) * progress;
            ...
            Main.EntitySpriteDraw(Ball, ballOff1, null, col2 with { A = 0 } * 0.35f, projectile.rotation, Ball.Size() / 2f, new Vector2(0.7f, 0.7f) * projectile.scale * overallScale, SE);

            //Border
            for (int i = 0; i < 4; i++)
            {
                Main.EntitySpriteDraw(vanillaTex, drawPos + Main.rand.NextVector2Circular(2.5f, 2.5f), sourceRectangle,
                    col1 with { A = 0 } * 1f, projectile.rotation, TexOrigin, projectile.scale * 1.1f * overallScale, SE);
            }
        ...
        public override bool PreKill(Projectile projectile, int timeLeft)
        {
            if (projectile.type == ProjectileID.DryRocket)
            {
                ProjectileExtensions.DoBaseDryRocketExplosionFX(projectile, storedPosition);
            ...
            else if (projectile.type == ProjectileID.WetRocket)
            {
                ProjectileExtensions.DoBaseWetRocketExplosionFX(projectile, storedPosition);
            ...
            else if (projectile.type == ProjectileID.LavaRocket)
            {
                ProjectileExtensions.DoBaseLavaRocketExplosionFX(projectile, storedPosition);
            ...
            else if (projectile.type == ProjectileID.HoneyRocket)
            {
                ProjectileExtensions.DoBaseHoneyRocketExplosionFX(projectile, storedPosition);
            ...
            SoundStyle style = new SoundStyle("VFXPlus/Sounds/Effects/water_blast_projectile_spell_03") with { Volume = .15f, Pitch = -.05f, PitchVariance = 0.05f, MaxInstances = -1 };
            SoundEngine.PlaySound(style, storedPosition);
    ...
    public class ClusterRocketShardsOverride : GlobalProjectile
    {
        public override bool InstancePerEntity => true;

        public override bool AppliesToEntity(Projectile entity, bool lateInstantiation)
        {
            return lateInstantiation && (entity.type == ProjectileID.ClusterFragmentsI || entity.type == ProjectileID.ClusterFragmentsII);
        }
        ...
        public override bool PreDraw(Projectile projectile, ref Color lightColor)
        {
            ...
            for (int i = 0; i < previousRotations.Count; i++)
            {
                ...
                //Start End
                Color col = Color.Lerp(Color.Yellow, Color.Gray, 1f - Easings.easeInCubic(progress)) * progress;
            ...
            {
                Main.EntitySpriteDraw(vanillaTex, drawPos + Main.rand.NextVector2Circular(2.5f, 2.5f), sourceRectangle,
                    Color.Yellow with { A = 0 } * 0.75f, projectile.rotation, TexOrigin, projectile.scale * 1.1f * overallScale, SpriteEffects.None);
            }
        ...
        public override bool PreKill(Projectile projectile, int timeLeft)
        {
            ...
            {
                Vector2 vel = Main.rand.NextVector2CircularEdge(2f, 2f) * Main.rand.NextFloat(1f, 2f);

                Dust dp = Dust.NewDustPerfect(projectile.Center, ModContent.DustType<ElectricSparkGlow>(), vel, newColor: Color.Goldenrod, Scale: Main.rand.NextFloat(0.45f, 0.65f) * 1.35f);
                ...
                ElectricSparkBehavior esb = new ElectricSparkBehavior(FadeAlphaPower: 0.89f, FadeScalePower: 0.98f, FadeVelPower: 0.92f, Pixelize: true, XScale: 1f, YScale: 0.75f,
                    UnderGlowPower: 2f, WhiteLayerPower: 0.35f);
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                Dust gd = Dust.NewDustPerfect(projectile.Center, ModContent.DustType<GlowPixelCross>(), dustVel, newColor: Color.DarkGoldenrod, Scale: Main.rand.NextFloat(0.25f, 0.45f));
            ...
            SoundEngine.PlaySound(SoundID.Item14 with { Volume = 0.25f, Pitch = -0.1f, PitchVariance = 0.15f, MaxInstances = -1 }, projectile.position);
