...
namespace VFXPlus.Content.Weapons.Ranged.PreHardmode.Guns
{
    public class RedRyder : GlobalItem 
    {
        ...
        {
            return lateInstatiation && (item.type == ItemID.RedRyder);
        }
        ...
        public override bool Shoot(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            {
                held.SetProjInfo(
                    GunID: ItemID.RedRyder,
                    AnimTime: 25,
                    NormalXOffset: 18f,
                    DestXOffset: 4f,
                    YRecoilAmount: 0.2f,
                    HoldOffset: new Vector2(0f, 1f),
                    TipPos: new Vector2(32f, -3f),
                    StarPos: new Vector2(29f, -3f)
                    ); 
            }

            //Explosion
            int dir = velocity.X > 0 ? 1 : -1;
            Vector2 muzzlePos = position + new Vector2(40f, -2f * dir).RotatedBy(velocity.ToRotation()) + new Vector2(0f, 0f);

            //Vector2 muzzlePos = position + velocity.SafeNormalize(Vector2.UnitX) * 50f;
            for (int i = 0; i < 7; i++) //16
            {
                Color col1 = Color.Lerp(Color.OrangeRed, Color.Red, 0.15f);
            ...
            {
                Color col1 = Color.Lerp(Color.Red, Color.OrangeRed, 0.65f);
