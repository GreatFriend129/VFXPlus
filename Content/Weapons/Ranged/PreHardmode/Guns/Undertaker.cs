...
{
    public class Undertaker : GlobalItem 
    {
        public override bool InstancePerEntity => true;
        public override bool AppliesToEntity(Item item, bool lateInstatiation)
        {
            return lateInstatiation && (item.type == ItemID.TheUndertaker);
        }
        ...
        public override bool Shoot(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            Vector2 muzzlePos = position + new Vector2(36f, -5f * dir).RotatedBy(velocity.ToRotation()) + new Vector2(0f, 0f);

            for (int i = 0; i < 7; i++) 
            {
                Color col1 = Color.Lerp(Color.OrangeRed, Color.Red, 0.65f);
            ...
            //Light Dust
            Dust softGlow = Dust.NewDustPerfect(muzzlePos, ModContent.DustType<SoftGlowDust>(), Vector2.Zero, newColor: Color.Red, Scale: 0.1f);
            ...
            for (int i = 0; i < 1 + Main.rand.Next(0, 2); i++)
            {
                ...
                Dust dust = Dust.NewDustPerfect(muzzlePos, ModContent.DustType<GlowPixelCross>(), randomStart, newColor: Color.Red, Scale: Main.rand.NextFloat(0.25f, 0.5f) * 1.5f);
                dust.noLight = false;
            ...
            //Sound
            //SoundStyle style1 = new SoundStyle("Terraria/Sounds/Custom/dd2_ballista_tower_shot_0") with { Pitch = .9f, PitchVariance = .25f, MaxInstances = -1, Volume = 0.1f };
            //SoundEngine.PlaySound(style1, position);


            SoundStyle style = new SoundStyle("Terraria/Sounds/Item_38") with { Volume = .6f, Pitch = 1f, PitchVariance = 0.2f };
            //SoundEngine.PlaySound(style, position);

            SoundStyle style3 = new SoundStyle("VFXPlus/Sounds/Effects/Gun/SniperShotA") with { Volume = .01f, Pitch = -1f, PitchVariance = 0.1f, MaxInstances = -1 };
            ...
            //Bullet Casing
            Gore.NewGore(source, position + velocity, new Vector2(velocity.X * -0.25f, -0.75f), ModContent.GoreType<BulletCasingSmallRed>());

            Vector2 bloodPos = position + new Vector2(28f, -6f * dir).RotatedBy(velocity.ToRotation()) + new Vector2(0f, 0f);
            for (int i = 0; i < 5 + Main.rand.Next(0, 3); i++)
            {

                Vector2 randomStart = Main.rand.NextVector2Circular(2.5f, 2.5f) * 1f;
                Dust dust = Dust.NewDustPerfect(bloodPos, DustID.Blood, randomStart, newColor: Color.White, Scale: Main.rand.NextFloat(1f, 1.2f));
                dust.velocity += velocity.SafeNormalize(Vector2.UnitX) * Main.rand.NextFloat(3f, 5.5f);
                dust.noGravity = true;
            }
