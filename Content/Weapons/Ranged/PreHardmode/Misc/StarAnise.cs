...
{
    public class StarAnise : GlobalProjectile
    {
        public override bool InstancePerEntity => true;

        public override bool AppliesToEntity(Projectile entity, bool lateInstantiation)
        {
            return lateInstantiation && (entity.type == ProjectileID.StarAnise);
        }
        ...
        {            
            ...
            if (timer % 5 == 0 && Main.rand.NextBool(2) && timer > 5)
            {
                ...
                Dust dp = Dust.NewDustPerfect(pos, ModContent.DustType<MuraLineDust>(), vel * 1f, newColor: Color.SaddleBrown * 1f, Scale: Main.rand.NextFloat(0.3f, 0.65f) * 0.3f);
                dp.alpha = 13;
        ...
        public override bool PreDraw(Projectile projectile, ref Color lightColor)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                Main.EntitySpriteDraw(vanillaTex, drawPos + Main.rand.NextVector2Circular(2.5f, 2.5f), sourceRectangle,
                    Color.SaddleBrown with { A = 0 } * 0.55f, projectile.rotation, TexOrigin, projectile.scale * 1.1f * overallScale, SpriteEffects.None);
            }
        ...
        public void DrawTrail(Projectile projectile, bool giveUp)
        {
            ...
            Color thisGray = Color.SaddleBrown;
            ...
            for (int i = 0; i < previousRotations.Count; i++)
            {
                ...
                //Start End
                Color col = Color.Lerp(thisGray, Color.SaddleBrown, 1f - Easings.easeInCubic(progress)) * progress;
                ...
                if (i > 1)
                {
                    ...
                    Main.EntitySpriteDraw(flare, AfterImagePos, null, thisGray with { A = 0 } * 0.2f * middleProg,
        ...
        public override bool PreKill(Projectile projectile, int timeLeft)
        {
            for (int i = 0; i < 3 + Main.rand.Next(0, 2); i++)
            {
                ...
                Dust dp = Dust.NewDustPerfect(projectile.Center, ModContent.DustType<MuraLineDust>(), vel.RotateRandom(0.5) * 1f, newColor: Color.SaddleBrown * 1f, Scale: Main.rand.NextFloat(0.3f, 0.65f) * 0.3f);
                dp.alpha = 13;
            ...
            for (int num631 = 0; num631 < 6; num631++)
            {
                int a = Dust.NewDust(projectile.position, projectile.width, projectile.height, 0, projectile.velocity.X * 0.1f, projectile.velocity.Y * 0.1f, 0, default(Color), 0.7f);
        ...
        public override void OnHitNPC(Projectile projectile, NPC target, NPC.HitInfo hit, int damageDone)
        {
            for (int i = 0; i < 4; i++)
            {
                ...
                Dust dust = Dust.NewDustPerfect(projectile.Center, ModContent.DustType<GlowPixelCross>(), randomStart, newColor: Color.SaddleBrown, Scale: Main.rand.NextFloat(0.35f, 0.45f));
