...
{    
    ...
    float2 chainCoords = float2((baseCoords.x * reps) + progress, baseCoords.y);
    ...
    float4 toPosterize = in_color * input.Color;
    
    toPosterize.rgb = Posterize(toPosterize.rgb);
    
    float alpha = 1.0; //round(toPosterize.a * posterizationSteps) / posterizationSteps;
    
    if (length(toPosterize.rgb) == 0.0)
        alpha = 0.0;
    
    if (posterizationSteps <= 0.0)
        return float4(toPosterize.rgb, alpha * length(toPosterize.rgb)) * totalMult;
    else
        return float4(toPosterize.rgb, alpha) * totalMult;
